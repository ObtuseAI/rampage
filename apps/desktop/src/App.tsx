import { Activity, Boxes, BrainCircuit, CircleStop, Command, Grid2X2, Orbit, RefreshCw, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="shell">
      ...
      <header className="topbar">
        ...
        <div className="status-ribbon" role="status"><i className={state.connected ? "online" : "reduced"} /><strong>{state.fabricRole === "worker" ? "WORKER ACTIVE" : state.connected ? "FABRIC LIVE" : "LOCAL REDUCED"}</strong><span>{state.nodes.length} nodes</span><span>{state.meshMode.replace("_", " ")}</span><span>{state.capability.replaceAll("_", " ")}</span></div>
      ...
      <main id="main">
        ...
        <aside className="inspector" aria-live="polite">
          ...
          <dl><div><dt>Memory</dt><dd>{selected.memory}%</dd></div><div><dt>GPU</dt><dd>{selected.gpu}%</dd></div><div><dt>Donated storage</dt><dd>{selected.storageAvailableGb} GB free</dd></div><div><dt>Fabric link</dt><dd>{selected.topologyConfidence === "measured" ? `${selected.latencyMs} ms / ${selected.downlinkMbps} Mbps down` : selected.topologyConfidence === "controller_local" ? "Controller local" : "Awaiting signed benchmark"}</dd></div><div><dt>Kind</dt><dd>{selected.kind.replace("_", " ")}</dd></div><div><dt>Policy</dt><dd>Owner protected</dd></div></dl>
